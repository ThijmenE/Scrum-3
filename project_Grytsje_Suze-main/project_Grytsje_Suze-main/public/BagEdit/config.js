export const materialPresets = {
  ...
  hardware: { metalness: 0.99, roughness: 0.05, envIntensity: 1.4 },
...
export async function loadModelConfig(modelName) {
  ...
  const json = res.ok ? await res.json() : await (async () => {
    const fallback = await fetch(CONFIG_BASE + 'configs/default.json');
    if (!fallback.ok) throw new Error(`Failed to load config: ${path}`);
    return fallback.json();
  })();

  // merge any per-bag materialPresets defined in the JSON into the global table
  if (json._materialPresets) {
    Object.assign(materialPresets, json._materialPresets);
    delete json._materialPresets;
  }
